...
class gz::sim::systems::AdvancedLiftDragPrivate
{
  ...
  public:
	...
	/// \brief Thread for spinning the ROS 2 executor
...
// Destructor implementation of Jan
AdvancedLiftDrag::~AdvancedLiftDrag()
{
...
void AdvancedLiftDragPrivate::Update(EntityComponentManager &_ecm)
{
  ...
  // show the link position including attitude in Jan debug GZ_LOG
  // gzdbg << "Link Position: X: " << pose.Pos().X() << " Y: " << pose.Pos().Y()
  //       << " Z: " << pose.Pos().Z() << "\n" << "Rotation: " << pose.Rot() << "\n";
  // show the body frame in Jan debug GZ_LOG
  // gzdbg << "Body Frame: X: " << body_x_axis << " Y: " << body_y_axis
  //       << " Z: " << body_z_axis << "\n";
...
//   gzdbg << "Speed in Lift-Drag Plane: " << speedInLDPlane << "\n";
  ...
  // Jan debug GZ_LOG
//   gzdbg << "Computed Angle of Attack (alpha): " << this->alpha << "\n";
                         ...
                         EntityComponentManager &_ecm, EventManager &)
{
  ...
  this->dataPtr->rosNode = rclcpp::Node::make_shared("advanced_lift_drag_plugin");
